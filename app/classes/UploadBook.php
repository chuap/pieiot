...
class UploadBook
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/'.Session::get('conf.uploadURL'),
            'upload_url' => $this->get_full_url().'/files/'.Session::get('conf.uploadURL'),
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function set_additional_file_properties($file) {
        //$file->deleteUrl = $this->options['script_url']
				//chuap fileupload
        $file->deleteUrl = $this->options['script_url']."/uploadbook"
				    .$this->get_query_separator($this->options['script_url'])
    ...
    protected function trim_file_name($file_path, $name, $size, $type, $error,
        ...
        $tname=$name;
        //chuap
        Session::put('lfname', $tname);        
        //$lid=FilesBook::lastid(); 
        
        return  rand(1,9999999999).'.'.pathinfo($tname, PATHINFO_EXTENSION);
        //return  $lid.'.'.pathinfo($tname, PATHINFO_EXTENSION);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            //chuap: For add DB script 
            FilesBook::addNewFile($file,$file_path);           
    ...
    public function delete($print_response = true) {
	    ...
	    //chuap : add DB script for delete file
            ...
            if ($success) {
                foreach($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
                        ...
                        //chuap
                        FilesBook::deleteFile($file_name); 
                        if (is_file($file)) {
                            unlink($file); 
                        }
                    }
