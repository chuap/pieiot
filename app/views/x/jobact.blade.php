...
?>
@section('breadcrumbs')
<li class="active">สถานะงานปัจจุบัน {{Session::get('cat.department')}}</li>
@stop
@section('body')
<div class="row-fluid">
    <!-- main col left --> 
    <div class="span12">
        <div class="row-fluid">
            <h3 class="header smaller lighter blue">สถานะงานปัจจุบัน <small>{{Session::get('cat.department')}}</small>
                <span class="label label-large arrowed-right label-{{$css}}">{{$type_name}}</span>
                <div class="pull-right f16">
                    @if($page>0)
                    <a class="btn btn-mini" href="{{asset("edoc/jobact?y=$y&m=$m&page=".($page))}}"><i class="icon-chevron-left"></i> ก่อนหน้า</a>
                    @endif
                    @if(count($alllist)>=$pcount)
                    <a class="btn btn-mini" href="{{asset("edoc/jobact?y=$y&m=$m&page=".($page+2))}}">หน้าถัดไป <i class="icon-chevron-right"></i></a>
                    @endif
                </div>
                <div class="btn-group">
                    <ul class="dropdown-menu dropdown-default w300">
                        <li class="{{$faqtype?'':'active'}}">
                            <a class="" href="{{asset("edoc/jobact")}}">ข้อมูลทั้งหมด</a>
                        </li>
                        @foreach(EDoc::faqtype()as $d)
                        <li class="{{$faqtype==$d->efrom?'active':''}}">
                            <a class="" href="{{asset('edoc/jobact?faqtype='.$d->efrom)}}">{{$d->efrom?$d->efrom:'<ไม่ระบุ>'}}<span class="badge badge- pull-right">{{$d->co}}</span></a>
                        </li>
                        @endforeach
                    </ul>                    
                    <button data-toggle="dropdown" class="btn btn- dropdown-toggle">
                        <i class="icon-star"></i> 
                        {{$faqtype?$faqtype:'เลือกส่วนงาน'}}
                        <span class="caret"></span>
                    </button>
                </div>
                <small><a href="{{asset('edoc/new?etype=job')}}" class="green"><i class="icon-pencil"></i> สร้างงานใหม่</a></small>
            </h3>
                        ...
                        <td>
                            <div class="mb1 alert alert-info f14 pr1">
                                ...
                                <small class="grey pull-right">
                                    <i class="icon-star"></i> <a class="green" href="{{asset('edoc/jobact?faqtype='.$d->efrom)}}">{{$d->efrom}}</a>
                                    </small>
                                @endif
                            </div>
